...
try {
  (function () {
    ...
    const blobMat = new THREE.ShaderMaterial({
      uniforms: bU, vertexShader: bVert, fragmentShader: bFrag,
      transparent: true, depthWrite: false, blending: THREE.AdditiveBlending
    });
    const shellU = { uTime: bU.uTime, uAmp: { value: 0.55 }, uOpacity: { value: 0.16 }, uColorA: { value: CYAN.clone() }, uColorB: { value: BLUE.clone() } };
    const shellMat = new THREE.ShaderMaterial({
      uniforms: shellU, vertexShader: bVert, fragmentShader: bFrag,
      transparent: true, depthWrite: false, wireframe: true
    });

    const blobGroup = new THREE.Group();
    blobGroup.add(
      new THREE.Mesh(new THREE.SphereGeometry(2.1, isMobile ? 48 : 96, isMobile ? 48 : 96), blobMat),
      new THREE.Mesh(new THREE.SphereGeometry(2.1, 20, 20), shellMat)
    );
    blobGroup.position.x = isMobile ? 0 : 3.2;
    scene.add(blobGroup);

    [[3.4, BLUE, 0.28], [4.0, CYAN, 0.16]].forEach(([r, c, o], i) => {
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(r, 0.007, 8, 100),
        new THREE.MeshBasicMaterial({ color: c, transparent: true, opacity: o })
      );
      ring.rotation.x = Math.PI / 2 + (i ? -0.4 : 0.3);
      blobGroup.add(ring);
    });
    ...
    (function tick() {
      ...
      bU.uTime.value = t;
      bU.uAmp.value  = 0.55 + Math.sin(t * 0.7) * 0.1 + my * 0.2;
      const fade = Math.max(0, 1 - smoothT * 3.2);
      bU.uOpacity.value   = fade;
      shellU.uOpacity.value = 0.16 * fade;
      blobGroup.rotation.y = t * 0.12 + mx * 0.5;
      blobGroup.rotation.x = my * 0.3;
      blobGroup.scale.setScalar(0.6 + fade * 0.4);
