...
try {
  (function () {
    ...
    function shapeSphere() { const a=new Float32Array(P*3); for(let i=0;i<P;i++){const u=Math.random()*2-1,th=Math.random()*Math.PI*2,r=5.4,s=Math.sqrt(1-u*u);a[i*3]=r*s*Math.cos(th);a[i*3+1]=r*u;a[i*3+2]=r*s*Math.sin(th)-2;} return a; }
    function shapeGrid()   { const a=new Float32Array(P*3),side=Math.ceil(Math.sqrt(P)); for(let i=0;i<P;i++){const x=i%side,y=Math.floor(i/side);a[i*3]=(x/side-.5)*16;a[i*3+1]=(y/side-.5)*9;a[i*3+2]=-3+Math.sin(x*.5)*Math.cos(y*.5)*.8;} return a; }
    function shapeHelix()  { const a=new Float32Array(P*3); for(let i=0;i<P;i++){const t=i/P*Math.PI*8,x=i/P*16-8,s=i%2?1:-1;a[i*3]=x;a[i*3+1]=Math.sin(t)*1.6*s;a[i*3+2]=Math.cos(t)*1.6*s-2;} return a; }
    function shapeDiamond(){ const a=new Float32Array(P*3); for(let i=0;i<P;i++){let x=Math.random()*2-1,y=Math.random()*2-1,z=Math.random()*2-1;const m=Math.abs(x)+Math.abs(y)+Math.abs(z)||1,r=4.6;a[i*3]=x/m*r;a[i*3+1]=y/m*r;a[i*3+2]=z/m*r-2;} return a; }
    function shapeKnot()   { const a=new Float32Array(P*3); for(let i=0;i<P;i++){const t=i/P*Math.PI*2,p=2,q=3,r=2.6,rr=Math.cos(q*t)+2;a[i*3]=r*rr*Math.cos(p*t)*.8;a[i*3+1]=r*rr*Math.sin(p*t)*.8;a[i*3+2]=r*Math.sin(q*t)-2;} return a; }
